  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-1.5">
        {stepLog.filter(s => team.agentIds.includes(s.agentId)).slice(-20).map((step, i) => (
          <div key={`${step.timestamp}-${i}`} className="p-2 bg-white/[0.03] rounded-lg border border-white/5 text-[10px] font-mono">
            <div className="flex justify-between mb-0.5">
              <span className="text-purple-400">{agents.find(a => a.id === step.agentId)?.name ?? step.agentId}</span>
