...
import { Capability, CapabilityCheckResult } from '../trust-types';
...
export abstract class BasePluginManager<TContext extends PluginContext = PluginContext> extends EventEmitter {
   ...
   */
  protected async loadPluginFromManifest(
    ...
    try {
        const context = this.createContext(manifest, capabilityDecisions);
